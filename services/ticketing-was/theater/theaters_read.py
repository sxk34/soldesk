...
def _fetch_bootstrap_from_db(movie_map):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT theater_id, address, created_at, updated_at FROM theaters ORDER BY theater_id ASC")
            theater_rows = cur.fetchall()

            cur.execute("""
                SELECT h.hall_id, h.theater_id, h.hall_name, h.total_seats, h.created_at, h.updated_at,
                ...
                GROUP BY h.hall_id, h.theater_id, h.hall_name, h.total_seats, h.created_at, h.updated_at
                ORDER BY h.theater_id ASC, h.hall_id ASC
            """)
            hall_rows = cur.fetchall()

            cur.execute("""
                SELECT schedule_id, movie_id, hall_id, show_date, total_count, remain_count, status, created_at, updated_at
                FROM schedules
